...
- **Implemented app behavior:** initialization, saved/live status, paginated largest-file and selected-directory size reports, worker controls and opt-in experimental metadata scanning; private TOML/SQLite state, exclusive writer lock, bounded batches, atomic inventory/job commits and directory reconciliation watermarks. Durable dispatch cadence/daily batch reservations, child-entry pacing with partial batches, live scanner accounting, WAL backpressure and versioned JSON commands are implemented. Fine-grained CPU/I/O/power enforcement, service installation and cleanup remain unavailable; the first review-candidate report is being validated.
- **Active task:** P2-01a/P2-02a in progress: bounded saved-inventory node_modules candidates with review-required evidence and measured sizes.
...
**Completed this session:** P2-01a/P2-02a implementation: `report --candidates` derives review-required old `node_modules` candidates from saved directory/manifest metadata. It includes local IDs, rule version, timestamps, regeneration cautions and bounded directory measurements. Filename-only project recognition is explicit; no manifest/content scan or action support. Pages examine 1,000 inventory entries and measure at most 20 subtrees with a five-second deadline; nested dependencies are suppressed. Docker check/race and four-target builds passed. Native macOS synthetic candidate smoke passed, including offline human/JSON evidence and expected sizes. CI verification is pending. No schema or dependency change.

**Next action:** finish native/CI validation of the candidate slice, then MVP-TRIAL on an explicitly selected real development folder. Prioritize useful feedback over unrelated resource infrastructure. The user also requested using an existing private CI host: read-only inspection found a persistent Docker-privileged private-repository runner. Runner routing has not changed; awaiting preference between free hosted CI and a separate trusted private workflow. Never register the shared host directly to the public repo or rely on editable PR workflow conditions as its trust boundary.
